...
<div class="container mt-5">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["id"])) {
        ...
        $sql = "SELECT * FROM student_schedule WHERE id_number = '$student_id'";
        ...
        if ($result->num_rows > 0) {
            ...
            ?>
            <h2>Edit Subject for <?= $schedule ?></h2>
            <form action="update_schedule.php" method="post">
                <input type="hidden" name="id" value="<?= $student_id ?>">
                <div class="mb-3">
                    <label for="schedule" class="form-label">New Schedule</label>
                    <input type="text" class="form-control" id="schedule" name="schedule" value="<?= $schedule ?>" required>
                </div>
